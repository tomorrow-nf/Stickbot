...
function configure() {
  ...
  if (!fs.existsSync('./info/roleEmoji.json')) {
    ...
    console.log(
      './info/roleEmoji.json created. This list is what gets added to any message in the role-assignment channel after it sees a :cgccWhite: react. Configured with the !emotelist command.'
    );
  }
  ...
  if (!fs.existsSync('./info/intros.json')) {
    ...
    console.log(
      './info/intros.json created. This should be a comma separated list of introduction strings to send when a new user joins.'
    );
  }
  ...
  if (didConfigure) {
    ...
    console.log(
      'The bot may not work properly until the created files are populated with accurate information, if applicable.'
    );
  }
}
