...
async function modCommands(message, args) {
  if (args[0] == '!members') {
    ...
    await message.channel.send(
      helpString[1] + '`!help` - Commands for all users.\n'
    );
    ...
    await message.channel.send(
      "CGCC has a number of controller modding skill roles! Here, you can use Discord reactions to assign your server-wide roles. Just click on the color-coded emote reactions below, and you'll be automatically assigned a role. To remove a role from yourself, just react again!"
    );
    await message.channel.send({
      files: [
        {
    ...
    await message.channel.send(
      '*Your primary role is your strongest skill, and also sets your name color. Secondary roles are for anything else you work with or dabble in!*'
    );
    await message.channel.send({
      files: [
        {
    ...
    if (args.length > 1) {
      ...
      return await message.channel.send(
        'Example usage: `!emotelist zhuW zhuW2 puffWhat`.'
      );
    }
...
async function userCommands(message, args) {
  ...
  // End of CHALLENGE commands
    ...
    let userHelpString =
      '`!contribute` - Allows you to submit a single post to #resources\n';
    userHelpString +=
      '`!challenge` - Use `!challenge help` to view Challenge commands\n';
    for (let i = 0; i < userCommandList.length; i++) {
      if (!userCommandList[i].hide) {
        userHelpString +=
          '`' +
